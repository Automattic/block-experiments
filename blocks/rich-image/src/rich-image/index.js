...
import { RotateLeftIcon, RotateRightIcon, FlipHorizontalIcon, FlipVerticalIcon, CropIcon, FilterIcon } from './icon';
...
class RichImage extends Component {
	...
	applyFilter = ( imageFilter ) => {
		...
		const adjustedAttrs = { imageFilter, ...getFilterClass( imageFilter, className ) };
		...
		return (
			<Fragment>
				...
				<div className={ classes }>
					{ inProgress && (
						<Fragment>
							<div className="richimage__working-notice">{ __( 'Processing…' ) }</div>
						</Fragment>
					) }
					...
					<CroppedImage
						...
						setCrop={ ( newCrop ) => this.setState( { crop: newCrop } ) }
					...
					<BlockControls>
						...
						<Toolbar>
							...
							<IconButton
								...
								icon={ <CropIcon /> }
								label={ __( 'Crop' ) }
								onClick={ () => this.setState( { isCrop: ! isCrop, crop: DEFAULT_CROP } ) }
							/>
							<IconButton
				...
				{ isCrop && <BlockControls>
					<Toolbar>
						<div className="richimage__crop-icon">
							<Icon icon={ CropIcon } />
						</div>
						<Button onClick={ this.cropImage }>{ __( 'Apply' ) }</Button>
						<Button onClick={ () => this.setState( { isCrop: false } ) }>{ __( 'Cancel' ) }</Button>
					</Toolbar>
				</BlockControls> }

				{ isFilter && (
					<Modal className="richimage__filter-modal" title={ __( 'Photo Filters' ) } onRequestClose={ () => this.setState( { isFilter: false } ) }>
						<FilterMenu
